...
export function Revenue() {
  ...
  const { data: walletBalance, isLoading: isLoadingWallet } = useWalletBalance();
  ...
  const filteredTransactions = transactions?.filter((transaction) => {
    ...
    if (filters.transactionStatus.length > 0 && !filters.transactionStatus.includes(transaction.status)) {
      return false;
    }
  ...
  const formattedTransactions =
    filteredTransactions?.map((transaction) => ({
      id: transaction?.payment_reference || transaction.amount + transaction.date,
      title: transaction?.metadata?.product_name || transaction?.metadata?.type || "Cash withdrawal",
      ...
      type: (transaction?.type === "deposit" ? "incoming" : "outgoing") as "incoming" | "outgoing",
    })) || [];
          ...
          {isLoadingWallet ? (
            ...
            <>
              ...
              <BalanceCard
                ...
                infoIcon
              />
              <BalanceCard title="Total Revenue" amount={formatCurrency(walletBalance?.total_revenue || 0)} infoIcon />
              <BalanceCard
